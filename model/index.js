...
class User{
    login(req, res) {
        ...
        `
        SELECT 
        userID ,firstName ,lastName , gender,emailAdd,userPass, userRole, userProfile
        FROM Users
        ...
        db.query(strQry,async(err,data) => {
            if (err) throw err;
            if((!data)||(data == null)){
                res.status(401).json({err:"You wrong email address"})
            }else{
                await compare(userPass, data[0].userPass,(cErr,cResult)=>{
                    ...
                    if(cResult){
                        res.status(200).json({
                            msg: 'Logged in', jwToken, result: data[0]
                        })
                    }else{
                        res.status(401).json({
